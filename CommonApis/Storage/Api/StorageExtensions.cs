...
public static class StorageExtensions {
    ...
    public static long Insert<TResource>(this IStorage<TResource> storage, ITransactionContext transaction, TResource resource, long amount, Storages.SlotSelectionOrder insertOrder = Storages.SlotSelectionOrder.First) where TResource : IResource<TResource> {
    ...
    /// </summary>
    public static long InsertMerging<TResource>(this IStorage<TResource> storage, ITransactionContext transaction, TResource resource, long amount, Storages.SlotSelectionOrder insertOrder = Storages.SlotSelectionOrder.First) where TResource : IResource<TResource> {
        return DoInsert(amount, 
    ...
    public static TResource? TrySwapResource<TResource>(this IStorage<TResource> storage, ITransactionContext transaction, TResource newResource, int slot = 0) where TResource : IResource<TResource> {
        using var nested = transaction.OpenNested();
    ...
    private static IEnumerable<IStorage<TResource>> GetSlotsInOrder<TResource>(this IStorage<TResource> storage, Storages.SlotSelectionOrder order) where TResource : IResource<TResource> => order switch {
        Storages.SlotSelectionOrder.First => Enumerable.Range(0, storage.SlotCount).Select(storage.Slot),
        Storages.SlotSelectionOrder.NonEmptyFirst => storage.NonEmptySlots().Concat(storage.EmptySlots()),
        Storages.SlotSelectionOrder.EmptyFirst => storage.EmptySlots().Concat(storage.NonEmptySlots()),
        Storages.SlotSelectionOrder.NonEmptyOnly => storage.NonEmptySlots(),
        Storages.SlotSelectionOrder.EmptyOnly => storage.EmptySlots(),
